...
Attached original image files are saved under `originals/<title>/`, outside the GitHub Pages deploy directory. Public JPEG tile thumbnails are generated only for the selected memo thumbnail under `docs/assets/posts/<title>/thumbs/`, and public detail images are generated for every attached image under `docs/assets/posts/<title>/display/`. The memo `images` field stores those resized detail images, while `originalImages` stores raw.githubusercontent.com links to the undeployed originals. Detail page images open the resized image first; the lightbox's `フルサイズ画像` button opens the raw GitHub original in a new tab. Approved posts are inserted above older non-fixed posts, while fixed entries such as `これは何？` remain first.
